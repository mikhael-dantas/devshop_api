...
export const ApiValidSchemas = {
  users: {
    ...
    post: schema.create({
      ...
      password: schema.string({}, [rules.minLength(6), rules.maxLength(180), rules.regex(/^\S*$/)]),
    }),
    ...
    put: schema.create({
      ...
      is_admin: schema.boolean.optional([rules.requiredIfNotExistsAll(['money_to_add'])]),
    }),
  },
  ...
  walletpayments: {
    post: schema.create({
      service_order_id: schema.number([rules.integer()]),
    }),
  },
}
